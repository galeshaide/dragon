...
# ✅ Load the trained model and scaler (fallback if missing)
...
try:
    ...
    print("✅ Model and Scaler loaded (if available)")
except Exception as e:
    model = None
    scaler = None
    print(f"⚠️ Could not load breast model/scaler ({e}) — using mock responses.")
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        # If model or scaler missing, return mock result
        if model is None or scaler is None:
            return {"prediction": "Benign", "note": "mock prediction - model/scaler not loaded"}
