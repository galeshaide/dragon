...
# ✅ Load the trained model (fallback to mock)
...
try:
    if os.path.exists(MODEL_PATH):
        ...
        print(f"⚠️ Diabetes model not found at {MODEL_PATH} — responses will be mocked.")
except Exception as e:
    model = None
    print(f"⚠️ Failed to load diabetes model ({e}) — responses will be mocked.")

# ✅ Get feature order from model (use a sensible default if model missing)
feature_order = model.feature_names_in_ if model is not None else ["BMI", "HighBP", "Age"]
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        if model is None:
            return {"prediction": "Low (mock)", "note": "mock prediction - model not loaded"}
