...
# Try to import PyTorch; if unavailable, use mock responses
TRY_TORCH = True
try:
    import torch
...
# ✅ Load trained model (if available) — otherwise use mock responses
MODEL_PATH = os.path.join(os.path.dirname(__file__), "pdac_cnn_v3.pth")
model = None
if torch is not None and os.path.exists(MODEL_PATH):
    ...
    print(f"⚠️ PDAC model not loaded (looked for {MODEL_PATH}) or torch unavailable — using mock responses.")

# ✅ Define transformations (only when torchvision available)
if TRY_TORCH:
...
async def predict(file: UploadFile = File(...)):
    if not file.filename.lower().endswith((".jpg", ".jpeg", ".png")):
        return JSONResponse(status_code=400, content={"error": "Only JPG, JPEG, or PNG files are supported"})
    ...
    # If model or torch not available, return a mock prediction
    if model is None or not TRY_TORCH:
    ...
    try:
        ...
        """
        return JSONResponse(status_code=200, content={"prediction": result_text.strip()})
    except Exception as e:
        return JSONResponse(status_code=500, content={"error": f"Model inference failed: {e}"})
